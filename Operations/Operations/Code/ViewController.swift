...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        disableStartButton()
        inputTextField.placeholder = "Input \(maxTextLength) character password"
    ...
    @IBAction func generatePasswordButtonPressed(_ sender: UIButton) {
        clearText()
        inputTextField.text = passwordGenerate.randomString(length: maxTextLength)
        enableStartButton()
    }
    ...
    private func start() {
        ...
        for i in 0...characterArray.count - 2 {
            let findOperation = BruteForceOperation(startIndex: i, endIndex: i+1, password: password)
            //По завершении операции проверяем нашелся ли пароль
            findOperation.completionBlock = {
                if let foundPassword = findOperation.foundPass {
